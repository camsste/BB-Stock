...
const ordemData = require('../data/ordem.json');

const orders = ordemData.copyWithin();
...
router.post('/', (req, res) => {
  ...
  const newOrder = {
    id: orders.length + 1, // Simula um id único
    data_ordem,
    data_necessidade,
    observacao,
    items
...
router.delete('/:id', (req, res) => {
  ...
  orders.splice(orderIndex, 1); // Deleta o item do array
  res.status(204).send();
});
